...
const UnitPage: React.FC<UnitPageProps> = ({ unit, progress, onUpdateProgress, onBack }) => {
  ...
  // ✅ PASO 1: Blindaje + orden por "order"
    ...
    <div className="pb-32">
          ...
          <div className="flex-1">
            ...
            <div className="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden">
              <div
                ...
                style={{ width: `${totalBlocks > 0 ? (visitedCount / totalBlocks) * 100 : 0}%` }}
              ></div>
            </div>
          </div>
      ...
      <div className="mt-20 space-y-8">
        <header className="space-y-4 py-8 border-b border-slate-100">
          <button
            onClick={onBack}
            className="text-xs font-bold uppercase tracking-widest text-slate-400 hover:text-slate-900 flex items-center gap-2 transition-colors no-print"
          >
        ...
        <section className="max-w-3xl mx-auto pt-12 text-center border-t border-slate-100 no-print">
          <div
            className="inline-flex items-center gap-3 text-slate-400 group cursor-pointer hover:text-slate-800 transition-colors"
            onClick={onBack}
          >
